...
class TaskMember(Base):
    ...
    task_id = Column(Integer, nullable=False, index=True)  # Related Task Kind.id
    user_id = Column(Integer, nullable=False, index=True)  # Member user ID
    invited_by = Column(Integer, nullable=False)  # Inviter user ID
    ...
    __table_args__ = (
        UniqueConstraint("task_id", "user_id", name="uq_task_member"),
        {

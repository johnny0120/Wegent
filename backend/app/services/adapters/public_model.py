...
class ModelAdapter:
    """
    Adapter to convert Kind (public model) to Model-like object for API compatibility
    """

    @staticmethod
    def to_model_dict(kind: Kind) -> Dict[str, Any]:
        """
        Convert Kind (public model) to Model-like dictionary
        """
...
class PublicModelService(BaseService[Kind, ModelCreate, ModelUpdate]):
    """
    Public Model service class - now uses kinds table with user_id=0
    """
    ...
    def create_model(
        ...
        """
        # Ensure unique name in default namespace for public models (user_id=0)
        existed = (
            db.query(Kind)
            .filter(
                Kind.user_id == 0,
        ...
        db_obj = Kind(
            user_id=0,  # Public models use user_id=0
            kind="Model",
            name=obj_in.name,
            namespace="default",
            json=json_data,
    ...
    def get_models(
        ...
        return [ModelAdapter.to_model_dict(m) for m in models]
    ...
    def list_model_names(
        ...
        Queries both user's own models (kinds table) and public models (kinds table with user_id=0).
        ...
        """
        # Get shell configuration from kinds table (public shells: user_id=0, kind='Shell')
        shell_row = (
            db.query(Kind.json)
            .filter(
                Kind.user_id == 0,
        ...
        # Query public models from kinds table (user_id=0)
        public_models = (
            db.query(Kind)
            .filter(
                Kind.user_id == 0,

...
export async function pollNewMessages(
  ...
  const response = await client.get<PollMessagesResponse>(
...
export async function getStreamingStatus(
  ...
  const response = await client.get<StreamingStatus>(
    `/subtasks/tasks/${taskId}/streaming-status`
  )
  return response.data
}
...
export function subscribeGroupStream(
  ...
  const url = `${baseURL}/subtasks/tasks/${taskId}/stream/subscribe?${params.toString()}`
